...
In the WebStorm menu `Run` select `Edit Configurations...`. Then click `+` and select `JavaScript Debug`. Paste `http://localhost:3000` into the URL fieldEntity and save the configuration.
...
Next we add a 'lint-staged' fieldEntity to the `package.json`, for example:
...
To tell the development server to proxy any unknown requests to your API server in development, add a `proxy` fieldEntity to your `package.json`, for example:
...
provides more context about what each fieldEntity means, and how your customizations
...
Open your `package.json` and add a `homepage` fieldEntity for your project:
...
Create React App uses the `homepage` fieldEntity to determine the root URL in the built HTML file.
